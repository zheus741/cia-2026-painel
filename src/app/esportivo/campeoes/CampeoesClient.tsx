 ...
 * campeões das 8 conferências, campeões por modalidade e a Liga Super 8.
 */

import React, { useMemo, useState } from 'react'
import Link from 'next/link'
import { Trophy, Crown, Medal, Zap, ArrowLeft, Sparkles } from 'lucide-react'
...
export function CampeoesClient({ divisoes, conferencias, modalidades, super8, totalModalidades }: Props) {
  ...
  return (
    <div className="mx-auto w-full max-w-[1500px] space-y-10 px-4 py-6 sm:px-6 md:py-10 lg:px-10">
      ...
      {/* ── Hero ─────────────────────────────────────────────────────────── */}
      <header className="cia-glass-gold relative overflow-hidden rounded-3xl border border-[var(--gold-dim)] px-6 py-10 text-center sm:px-10 sm:py-14">
        ...
        <p className="mx-auto mt-3 max-w-xl text-sm text-[var(--muted-foreground)] sm:text-base">
          Os títulos já conquistados — divisões, conferências e modalidades.
        </p>
        <div className="mt-6 flex flex-wrap items-center justify-center gap-3 text-sm">
          <span className="rounded-full bg-[var(--card)] px-4 py-1.5 font-semibold shadow-sm">{divisoes.length} divisões</span>
          <span className="rounded-full bg-[var(--card)] px-4 py-1.5 font-semibold shadow-sm">{conferencias.length} conferências</span>
          <span className="rounded-full bg-[var(--card)] px-4 py-1.5 font-semibold shadow-sm">{totalModalidades} títulos de modalidade</span>
        </div>
      </header>
      ...
      <section className="space-y-4">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          {divisoes.map(d => (
            <article key={d.nome} className="cia-card relative overflow-hidden rounded-2xl p-6"
              style={{ borderTop: `4px solid ${d.cor}` }}>
              <p className="text-xs font-bold uppercase tracking-[0.2em] text-[var(--muted-foreground)]">{d.nome}</p>
              {d.campeao ? (
                <>
                  <AtleticaLink slug={d.campeao.slug} className="mt-2 block text-2xl font-black leading-tight text-[var(--foreground)] hover:underline sm:text-3xl">
                    {d.campeao.nome}
                  </AtleticaLink>
                  {d.campeao.universidade && <p className="mt-1 text-sm text-[var(--muted-foreground)]">{d.campeao.universidade}</p>}
                  <div className="mt-3 inline-flex items-center gap-2 rounded-full px-3 py-1 text-sm font-bold"
                    style={{ background: `${d.cor}22`, color: d.cor }}>
                    <Trophy size={15} /> {d.campeao.pontos} pts
                  </div>
                </>
          ...
          <div className="cia-card rounded-2xl p-5">
            {super8.resumo.liga_finalizada && s8lider ? (
              ...
              <p className="mb-4 inline-flex items-center gap-2 rounded-full bg-[var(--muted)] px-3 py-1 text-sm font-semibold text-[var(--muted-foreground)]">
                <Sparkles size={14} /> Em andamento · {super8.resumo.jogos_encerrados}/{super8.resumo.total_jogos} jogos
              </p>
            )}
            <ol className="space-y-1">
              {super8.standings.slice(0, 8).map((s, i) => (
                <li key={s.atletica_id} className="flex items-center gap-3 rounded-lg px-2 py-1.5"
      ...
      <section className="space-y-4">
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {conferencias.map(c => (
            <article key={c.nome} className="cia-card relative overflow-hidden rounded-2xl p-5"
              ...
              {c.campeao ? (
                <>
                  <AtleticaLink slug={c.campeao.slug} className="mt-2 block text-xl font-black leading-tight hover:underline">
                    {c.campeao.nome}
                  </AtleticaLink>
                  <div className="mt-2 inline-flex items-center gap-1.5 text-sm font-bold" style={{ color: c.cor }}>
                    <Trophy size={14} /> {c.campeao.pontos} pts
                  </div>
                </>
      ...
      <section className="space-y-4">
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {visiveis.map(grp => (
            <article key={grp.grupo} className="cia-card rounded-2xl p-5"
              style={{ borderTop: `4px solid ${grp.conf?.cor ?? (grp.grupo.includes('1ª') ? '#F0D04A' : grp.grupo.includes('2ª') ? '#4aa06a' : '#8a5f06')}` }}>
              <p className="mb-3 flex items-center gap-2 text-sm font-bold uppercase tracking-[0.15em]"
                style={{ color: grp.conf?.cor ?? 'var(--foreground)' }}>
                ...
                {grp.items.map((it, i) => (
                  <li key={i} className="flex items-baseline justify-between gap-3 text-sm">
                    <span className="text-[var(--muted-foreground)]">
                      {it.modalidade}{it.categoria ? <span className="opacity-70"> · {it.categoria}</span> : null}
                    </span>
