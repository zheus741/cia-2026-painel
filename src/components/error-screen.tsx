...
/**
 * ErrorScreen — usado pelos error.tsx das rotas críticas.
 * Padrão visual unificado, scope-aware (mostra qual área falhou).
 */
...
interface Props {
  ...
  unstable_retry: () => void
  /** Nome da área que falhou — ex: "Placar", "Hub Esportivo". */
  scope?:         string
  /** Subtítulo customizado. Default: mensagem genérica. */
  subtitle?:      string
}

export function ErrorScreen({ error, unstable_retry, scope, subtitle }: Props) {
  ...
  return (
    ...
    >
      ...
      >
        {scope ? `Erro · ${scope}` : 'Tempo técnico'}
      </p>
      <h1
        className="mt-2 max-w-md text-center text-2xl font-extrabold tracking-tight md:text-3xl"
        style={{ color: 'var(--foreground)', letterSpacing: '-0.02em' }}
      >
        {subtitle ?? 'A transmissão caiu por um segundo'}
      </h1>
      <p
        className="mt-3 max-w-sm text-center text-sm leading-relaxed"
        style={{ color: 'var(--muted-foreground)' }}
      >
        Provavelmente foi um soluço do banco ou da rede. Tenta de novo;
        se travar, manda o código abaixo pro Mateus.
      </p>
